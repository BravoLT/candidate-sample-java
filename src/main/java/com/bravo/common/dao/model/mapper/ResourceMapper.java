package com.bravo.common.dao.model.mapper;
...
public class ResourceMapper {
  ...
  public <T extends Collection<Payment>> List<PaymentReadDto> convertPayments(final T payments){
    return payments.stream().map(this::convertPayment).collect(Collectors.toList());
  }

  public PaymentReadDto convertPayment(final Payment payment){
    final String cardNumber = payment.getCardNumber();
    final PaymentReadDto dto = mapperFacade.map(payment, PaymentReadDto.class);
